...
cut_size = 44
bs = 1
models = ['VGG16_0/PrivateTest_model.t7', 'VGG19/PrivateTest_model.t7', 'ResNet18/PrivateTest_model.t7']
model_i = 0
model_path = models[model_i]

epls = [0.001 * i for i in range(15)]
...
def test(model, device, test_loader, eps):
    ...
    for data, target in test_loader:
        ...
        if apply_defend:
            perturbed_data = fgsm_defense(perturbed_data, 0.1, device)
...
if __name__ == '__main__':
    log_path = 'VGG16_0/fgsm_vgg16_0.1defend.txt'
    f = open(log_path, 'a')
    PublicTestset = FER2013(split='PublicTest', transform=transform_test)
    PublicTestloader = torch.utils.data.DataLoader(PublicTestset, batch_size=bs, shuffle=False)
    PrivateTestset = FER2013(split='PrivateTest', transform=transform_test)
    PrivateTestloader = torch.utils.data.DataLoader(PrivateTestset, batch_size=bs, shuffle=False)
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig('VGG16_0/vgg16_0.1defend.png')
    print('Finish')